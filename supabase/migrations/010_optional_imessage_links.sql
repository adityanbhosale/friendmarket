...
-- Run after 009. Sidebar groups remain valid without any row in these tables.
...
begin
  ...
  v_created := public.create_group_with_owner(
    ...
    p_admin_email, p_recovery_code_hash, p_starting_points
  );
...
revoke all on function public.create_group_with_owner_imessage(text, text, text, text, text, text, text, integer) from public, anon, authenticated;
...
grant execute on function public.create_group_with_owner_imessage(text, text, text, text, text, text, text, integer) to service_role;
