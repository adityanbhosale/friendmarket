...
create table public.group_members (
  ...
  identity_code text not null check (
    identity_code ~ '^SB-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$'
  ),
  phone_attached_at timestamptz default now(),
...
create table public.markets (
  ...
  constraint markets_subject_complete check (
...
create table public.imessage_pending_links (
...
create index imessage_pending_links_expiry_idx
  on public.imessage_pending_links (expires_at);

create table public.imessage_market_drafts (
...
create index imessage_market_drafts_expiry_idx
...
select
  ...
  s.side_id,
  case when now() >= m.reveal_at then sum(s.amount)::integer end as pool,
  case when now() >= m.reveal_at then round(
...
create function public.open_market_with_subject(
  p_group_id uuid, p_proposer_id uuid, p_question text, p_criteria text,
  p_reveal_at timestamptz, p_close_at timestamptz, p_resolve_at timestamptz,
  p_subject_name text default null, p_subject_phone_hash text default null,
  p_yes_label text default 'Yes', p_no_label text default 'No'
)
returns uuid language plpgsql as $$
...
begin
  ...
  order by (gm.user_id = p_proposer_id), random()
  limit 1;
  update public.markets set subject_name = nullif(btrim(p_subject_name), ''),
...
create function public.create_group_with_owner_phone(
  ...
  p_admin_email text, p_recovery_code_hash text,
  p_phone_hash text, p_identity_code text,
...
begin
  ...
  insert into public.group_members (group_id, user_id, phone_hash, identity_code)
    values (v_group_id, v_user_id, p_phone_hash, p_identity_code);
...
begin
  ...
  select gm.user_id, u.name into v_user_id, v_registered_name
  from public.group_members gm
  ...
  insert into public.users (name, recovery_code_hash)
    values (btrim(p_user_name), p_recovery_code_hash) returning id into v_user_id;
  insert into public.group_members (group_id, user_id, phone_hash, identity_code)
    values (p_group_id, v_user_id, p_phone_hash, p_identity_code);
  insert into public.points_ledger (group_id, user_id, delta, reason)
...
create function public.set_member_email(
  p_group_id uuid, p_user_id uuid, p_email text
)
returns void language plpgsql as $$
begin
  if not exists (
  ...
  if length(btrim(coalesce(p_email, ''))) not between 3 and 254
    or strpos(btrim(p_email), '@') <= 1 then
    raise exception 'invalid email address' using errcode = '22023';
  end if;
  update public.users set email = lower(btrim(p_email)) where id = p_user_id;
end;
$$;

create function public.attach_member_phone(
  p_group_id uuid, p_user_id uuid, p_phone_hash text, p_identity_code text
)
returns void language plpgsql as $$
begin
  ...
  update public.group_members
  set phone_hash = p_phone_hash, identity_code = p_identity_code,
      phone_attached_at = now()
  where group_id = p_group_id and user_id = p_user_id
...
create function public.create_group_with_owner_phone_imessage(
  ...
  p_user_name text, p_admin_email text, p_recovery_code_hash text,
  p_phone_hash text, p_identity_code text,
  p_starting_points integer default 1000
)
returns jsonb language plpgsql as $$
...
begin
  ...
  v_created := public.create_group_with_owner_phone(
    ...
    p_admin_email, p_recovery_code_hash, p_phone_hash, p_identity_code,
    p_starting_points
  );
...
create function public.enter_group_member_phone_imessage(
  ...
  p_recovery_code_hash text, p_phone_hash text, p_identity_code text,
  p_starting_points integer default 1000
)
returns jsonb language plpgsql as $$
...
begin
  ...
  v_entry := public.enter_group_member_phone(
    p_group_id, p_user_name, p_recovery_code_hash, p_phone_hash,
    p_identity_code, p_starting_points
  );
  perform public.consume_imessage_setup(
...
create function public.stage_imessage_web_link(
  p_sender_hash text, p_group_link_id text, p_phone_hash text,
  p_expires_at timestamptz
)
returns jsonb language plpgsql as $$
declare v_group public.groups%rowtype; v_user_id uuid;
begin
  ...
  select * into v_group from public.groups where link_id = p_group_link_id;
  if not found then raise exception 'no Sidebar group uses that code' using errcode = '22023'; end if;
  select user_id into v_user_id from public.group_members
  ...
  insert into public.imessage_pending_links (sender_hash, group_id, user_id, expires_at)
  values (p_sender_hash, v_group.id, v_user_id, p_expires_at)
  on conflict (sender_hash) do update
    set group_id = excluded.group_id, user_id = excluded.user_id,
        expires_at = excluded.expires_at, created_at = now();
  return jsonb_build_object(
    'group_id', v_group.id, 'user_id', v_user_id, 'group_name', v_group.name
  );
end;
$$;

create function public.claim_imessage_web_link(
  p_sender_hash text, p_conversation_hash text
)
returns jsonb language plpgsql as $$
declare
  v_pending public.imessage_pending_links%rowtype;
  v_existing_group uuid; v_existing_user uuid;
begin
  if p_sender_hash !~ '^[0-9a-f]{64}$'
  ...
  select * into v_pending from public.imessage_pending_links
  where sender_hash = p_sender_hash and expires_at > now() for update;
  if not found then return null; end if;
  insert into public.imessage_conversations (conversation_hash, group_id, created_by)
  values (p_conversation_hash, v_pending.group_id, v_pending.user_id)
  on conflict (conversation_hash) do nothing;
  select group_id into v_existing_group from public.imessage_conversations
  ...
  insert into public.imessage_identities (conversation_hash, sender_hash, user_id)
  values (p_conversation_hash, p_sender_hash, v_pending.user_id) on conflict do nothing;
  select user_id into v_existing_user from public.imessage_identities
  ...
  return jsonb_build_object('group_id', v_pending.group_id, 'user_id', v_pending.user_id);
end;
$$;

create function public.stage_imessage_market_draft(
  p_group_id uuid, p_user_id uuid, p_question text, p_criteria text,
  p_reveal_at timestamptz, p_close_at timestamptz, p_resolve_at timestamptz,
  p_subject_name text, p_expires_at timestamptz
)
returns void language plpgsql as $$
begin
  if not exists (
  ...
  if length(btrim(coalesce(p_question, ''))) not between 1 and 200
    ...
    or p_close_at <= now() or p_expires_at <= now() then
  ...
  on conflict (user_id) do update set
    group_id = excluded.group_id, question = excluded.question,
    criteria = excluded.criteria, reveal_at = excluded.reveal_at,
    close_at = excluded.close_at, resolve_at = excluded.resolve_at,
    subject_name = excluded.subject_name, expires_at = excluded.expires_at,
    created_at = now();
end;
$$;

create function public.complete_imessage_market_draft(
  p_group_id uuid, p_user_id uuid, p_subject_name text default null,
  p_subject_phone_hash text default null
)
returns uuid language plpgsql as $$
declare v_draft public.imessage_market_drafts%rowtype; v_market_id uuid;
begin
  delete from public.imessage_market_drafts where expires_at <= now();
  select * into v_draft from public.imessage_market_drafts
  ...
  v_market_id := public.open_market_with_subject(
    v_draft.group_id, v_draft.user_id, v_draft.question, v_draft.criteria,
    v_draft.reveal_at, v_draft.close_at, v_draft.resolve_at,
    p_subject_name, p_subject_phone_hash
  );
